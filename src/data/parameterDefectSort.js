import { parameterDefect } from './mock'
...
// Priority order for status-type fields
export const STATUS_ORDER = { EMERGENCY: 1, ABNORMAL: 2, NORMAL: 3, MAINTENANCE: 4 }

// ── Comparator branched by type — null/undefined always falls to bottom ────
...
// ── Selector — accepts { sortBy, sortDir }, returns sorted insights ─────────
// API migration: swap accessor(row) → query string passed to fetch()
export function getParameterDefectInsights({ sortBy = 'count', sortDir = 'desc' } = {}) {
  const cfg = SORT_CONFIG.find(c => c.field === sortBy) ?? SORT_CONFIG[0]
  const sorted = [...parameterDefect.insights.peakDefect.byType].sort((a, b) =>
    compare(cfg.accessor(a), cfg.accessor(b), cfg.type, sortDir)
  )
  return {
    ...parameterDefect.insights,
    peakDefect: { ...parameterDefect.insights.peakDefect, byType: sorted },
  }
}
